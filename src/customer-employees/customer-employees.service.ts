...
export class CustomerEmployeesService {
  ...
  async changePassword(employeeId: string, currentPassword: string, newPassword: string): Promise<{ message: string }> {
    try {
      ...
      // For regular logins (mustChangePassword === false), require the current password to match.
      // For first login (mustChangePassword === true), trust the authenticated session and SKIP
      // validating the temporary password to avoid issues with migrated hashes between environments.
