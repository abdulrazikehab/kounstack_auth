...
async function bootstrap() {
  ...
  try {
    ...
    // Guarantee GET /health for load balancers (respond before Nest router so it always works)
    expressApp.use((req: Request, res: Response, next: NextFunction) => {
      if (req.method === 'GET' && (req.path === '/health' || req.originalUrl?.split('?')[0] === '/health')) {
        return res.status(200).json({ success: true, service: 'auth', status: 'healthy', timestamp: new Date().toISOString() });
      }
      next();
    });
    // CRITICAL: Enable CORS FIRST before any other middleware to prevent duplicate headers
    // Verified custom domains (from core CustomDomain table, status ACTIVE) - fetched from core API
    let verifiedCustomDomainHosts: string[] = [];
    const fetchVerifiedDomains = async () => {
      try {
        const base = (process.env.CORE_API_URL || (process.env.FRONTEND_URL || 'https://kounworld.com') + '/api').replace(/\/$/, '');
        const url = `${base}/public/verified-domains`;
        const res = await fetch(url, { method: 'GET', headers: { 'Accept': 'application/json' } });
        if (res.ok) {
          const data = await res.json() as { domains?: string[] };
          verifiedCustomDomainHosts = Array.isArray(data.domains) ? data.domains.map((d: string) => d.toLowerCase()) : [];
          if (verifiedCustomDomainHosts.length > 0) logger.log(`[CORS] Loaded ${verifiedCustomDomainHosts.length} verified custom domain(s) from core`);
        }
      } catch (e) {
        // ignore
      }
    ...
    setInterval(fetchVerifiedDomains, 5 * 60 * 1000);
    ...
    const allowedOriginsList = [
      ...
      'https://kounworld.net',
      'https://www.kounworld.com',
      'https://www.kounworld.net',
      'https://app.kounworld.com',
      'https://app.kounworld.net',
    ...
    app.use((req: Request, res: Response, next: NextFunction) => {
      if (req.method === 'OPTIONS') {
        ...
        if (origin) {
          ...
          const originHost = (() => { try { return new URL(origin).hostname.toLowerCase(); } catch { return ''; } })();
          const isVerifiedCustom = originHost && verifiedCustomDomainHosts.includes(originHost);
          if (
    ...
    app.use(cors({
      origin: (origin: string | undefined, callback: (err: Error | null, allow?: boolean | string) => void) => {
        ...
        // Allow verified custom domains (from core CustomDomain table, status ACTIVE)
        try {
          const originHost = new URL(origin).hostname.toLowerCase();
          if (verifiedCustomDomainHosts.includes(originHost)) return callback(null, origin);
