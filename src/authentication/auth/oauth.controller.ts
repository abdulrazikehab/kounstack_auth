...
@Controller() // Prefix is now handled globally in main.ts
...
async googleAuthRedirect(@Req() req: any, @Res() res: Response) {
  try {
    ...
    // OAuth callbacks should always go to the main domain: https://kounworld.com/oauth/callback
...
private buildTenantFrontendUrl(subdomain: string): string {
  ...
  if (hostname.includes(secondaryDomain) || hostname.includes('kounworld.com')) {
    const domain = hostname.includes(secondaryDomain) ? secondaryDomain : 'kounworld.com';
