...
export class StaffService {
  ...
  async createStaff(tenantId: string, creatingUserId: string, staffData: CreateStaffDto) {
    ...
    const result = await this.prismaService.$transaction(async (tx: any) => {
      ...
      // NEW: Handle tab locks if provided
      if ((staffData as any).tabLocks && Array.isArray((staffData as any).tabLocks)) {
        for (const lock of (staffData as any).tabLocks) {
          if (lock.tabKey && lock.password) {
            const lockHash = await bcrypt.hash(lock.password, 12);
            await tx.staffTabLock.create({
              data: {
                userId: user.id,
                tenantId,
                tabKey: lock.tabKey,
  ...
  async getStaffUsers(tenantId: string, page: number = 1, limit: number = 50) {
    ...
    try {
      [staffUsers, total] = await Promise.all([
      ...
      this.logger.warn('Staff query failed, attempting basic fallback:', err.message);
      ...
      [staffUsers, total] = await Promise.all([
        this.prismaService.user.findMany({
          ...
          select: {
            ...
            // Omit staffTabLocks as it likely caused the error
          },
          skip,
          take: limit,
          orderBy: { createdAt: 'desc' },
  ...
  async getStaffUser(tenantId: string, staffUserId: string) {
    ...
    try {
      staffUser = await this.prismaService.user.findFirst({
        where: {
          id: staffUserId,
          tenantId,
          role: 'STAFF',
      ...
      const err = error as { code?: string; message?: string };
      this.logger.warn(`getStaffUser basic query failed, attempting basic fallback: ${err.message}`);
  ...
  /**
   * Configure per-tab password locks for a staff user.
   * Each lock is identified by a tabKey (e.g. "management-products").
   */
  async updateStaffTabLocks(
    tenantId: string,
    updatingUserId: string,
    staffUserId: string,
    locks: { tabKey: string; password?: string | null }[],
  ) {
    // Ensure target user is a staff member of this tenant
    ...
    if (!staffUser) {
      throw new NotFoundException('Staff user not found for this tenant');
    }

    for (const lock of locks) {
      const tabKey = lock.tabKey.trim();
      if (!tabKey) continue;

      if (!lock.password) {
        // Delete existing lock for this tab
        await this.prismaService.staffTabLock.deleteMany({
          where: { userId: staffUserId, tenantId, tabKey },
        });
        continue;
      }

      const hash = await bcrypt.hash(lock.password, 12);

      await this.prismaService.staffTabLock.upsert({
        where: {
          userId_tenantId_tabKey: {
            userId: staffUserId,
            tenantId,
            tabKey,
          },
    ...
    await this.logAuditEvent(
      updatingUserId,
      tenantId,
      'STAFF_TAB_LOCKS_UPDATED',
      staffUserId,
      'user',
      undefined,
      { locks: locks.map(l => ({ tabKey: l.tabKey, hasPassword: !!l.password })) },
      undefined,
    );
  ...
  /**
   * Verify if a tab is locked for the current staff user and, if so,
  ...
  async verifyTabLock(
    ...
    const lock = await this.prismaService.staffTabLock.findFirst({
      where: {
        userId,
        tenantId,
        tabKey,
    ...
    if (!lock) {
      // No lock configured for this tab
      return { locked: false };
    }

    if (!password) {
      // Lock exists but no password provided -> tell client it is locked
      return { locked: true };
    }

    const ok = await bcrypt.compare(password, lock.passwordHash);
    if (!ok) {
      throw new ForbiddenException('Invalid tab password');
    }
  ...
  async sendCredentials(tenantId: string, email: string, password?: string, inviteUrl?: string) {
    ...
    if (inviteUrl) {
      ...
      const loginUrl = `${process.env.FRONTEND_URL || 'https://kounworld.com'}/auth/login`;
